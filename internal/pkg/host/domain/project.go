...
import (
	...
	project2 "github.com/spaulg/solo/internal/pkg/host/domain/project"
	domain_compose "github.com/spaulg/solo/internal/pkg/host/domain/project/compose"
	domain_types "github.com/spaulg/solo/internal/pkg/shared/domain"
	compose3 "github.com/spaulg/solo/internal/pkg/shared/domain/project/compose"
...
func NewProject(projectFilePath string, config *Config, profiles []string) (*Project, error) {
	...
	projectOptions, err := cli.NewProjectOptions(nil,
		...
		cli.WithExtension(project2.ServiceWorkflowExtensionName, project2.NewServiceWorkflows()),
		cli.WithExtension(project2.ToolExtensionName, project2.NewTools()),
...
func (t *Project) GetMaxWorkflowTimeout(eventName string) time.Duration {
	...
	for _, serviceConfig := range t.compose.Services {
		serviceWorkflows := serviceConfig.Extensions[project2.ServiceWorkflowExtensionName].(compose3.ServiceWorkflows)
...
func (t *Project) Services() compose3.Services {
	return domain_compose.NewServices(t, t.compose)
}

func (t *Project) Tools() compose3.Tools {
	if t, ok := t.Extensions[project2.ToolExtensionName].(compose3.Tools); ok {
		return t
	}

	return compose3.Tools{}
}
...
func (t *Project) loadServiceExtensionDefaults() {
	...
	for serviceName, serviceConfig := range t.compose.Services {
		...
		v, ok := serviceConfig.Extensions[project2.ServiceWorkflowExtensionName]
		if !ok {
			v = project2.NewServiceWorkflows()
			serviceConfig.Extensions[project2.ServiceWorkflowExtensionName] = v
		}

		workflows := v.(compose3.ServiceWorkflows)
		for _, workflowName := range workflowcommon.WorkflowNames {
			if _, ok := workflows[workflowName.String()]; !ok {
				workflows[workflowName.String()] = compose3.ServiceWorkflowConfig{
