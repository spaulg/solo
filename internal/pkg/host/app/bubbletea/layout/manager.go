...
func (t *Manager) ResizeComponents(w int, h int, components ...*tea.Model) []tea.Cmd {
	var cmd tea.Cmd
	var cmds []tea.Cmd

	dimensions := t.CalculateDimensions(w, h)

	for id, model := range components {
		*model, cmd = (*model).Update(messages.ComponentSizeMsg{
			Width:  dimensions[id].ContentBoxWidth(),
			Height: dimensions[id].ContentBoxHeight(),
		})
