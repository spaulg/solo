...
type Component struct {
	...
	rowSelectCallback func(selectedRow RowID) func() tea.Msg
...
func DefaultStyles() Styles {
	return Styles{
		...
		Header:   lipgloss.NewStyle().Bold(true).Inline(true),
...
func WithRowSelectCallback(rowSelectCallback func(selectedRow RowID) func() tea.Msg) Option {
	return func(t *Component) {
		t.rowSelectCallback = rowSelectCallback
	}
}
...
func (t Component) Update(msg tea.Msg) (Component, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case key.Matches(m, t.keyMap.Enter):
			if t.rowSelectCallback != nil {
				cmds = append(cmds, t.rowSelectCallback(t.selectedRow))
			}
		}
...
func (t *Component) refreshView() {
	if t.dirty {
		...
		view.WriteString(renderedColumns + "\n")
		...
		if len(t.rows) > 0 {
			for _, row := range t.rows[t.offset:] {
				...
				view.WriteString(renderedRow + "\n")
				rowCount++
			}
		}

		t.view = t.styles.Box.Render(view.String())
		t.dirty = false
	}
}
