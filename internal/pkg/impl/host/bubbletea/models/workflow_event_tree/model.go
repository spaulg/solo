...
import (
	...
	"github.com/spaulg/solo/internal/pkg/impl/common/wms"
...
func (t Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case messages.WorkflowEventSelected:
		cmds = append(cmds, func() tea.Msg {
			...
			workflowEventDirectory := ".solo/workflow_logs/" + m.WorkflowEvent
			...
			for _, entry := range entries {
				...
				if eventName != "" {
					eventFile := ".solo/workflow_logs/" + m.WorkflowEvent + "/" + entry.Name() + "/" + entry.Name() + ".meta.json"
