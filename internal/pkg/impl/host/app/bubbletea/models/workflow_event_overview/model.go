...
import (
	...
	"github.com/spaulg/solo/internal/pkg/impl/host/app/bubbletea/models/workflow_event_output"
...
func NewModel() Model {
	return Model{
		layoutManager: layout2.NewLayoutManager(
			layout2.HorizontalLayoutDirection,
			[]layout2.Spec{
				layout2.NewPercentageLayoutSpec(25, lipgloss.NewStyle().Border(lipgloss.RoundedBorder())),
				layout2.NewFillLayoutSpec(lipgloss.NewStyle().Border(lipgloss.RoundedBorder())),
			},
		),
...
func (t Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case messages2.ComponentSizeMsg:
		...
		t.workflowEventTree, cmd = t.workflowEventTree.Update(messages2.ComponentSizeMsg{
		...
		t.workflowEventOutput, cmd = t.workflowEventOutput.Update(messages2.ComponentSizeMsg{
