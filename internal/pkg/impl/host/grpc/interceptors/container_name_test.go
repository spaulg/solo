...
func (t *ContainerNameTestSuite) TestSuccessfulContainerNameUnaryInterceptor() {
	...
	expectedFullContainerName := "full-test-container"
	...
	result, err := interceptor.ContainerNameUnaryInterceptor(ctx, req, info, func(ctx context.Context, _ any) (any, error) {
		...
		fullContainerName, ok := ctx.Value(ContainerName(FullContainerNameContextValueName)).(string)

		t.True(ok)
		t.Equal(expectedFullContainerName, fullContainerName)
...
func (t *ContainerNameTestSuite) TestSuccessfulContainerNameStreamInterceptor() {
	...
	err := interceptor.ContainerNameStreamInterceptor(srv, ss, info, func(_ any, stream grpc.ServerStream) error {
		...
		fullContainerName, ok := stream.Context().Value(ContainerName(FullContainerNameContextValueName)).(string)
