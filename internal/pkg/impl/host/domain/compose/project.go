...
func NewProject(projectFilePath string, config *domain.Config, profiles []string) (*Project, error) {
...
func (t *Project) Services() domain.Services {
	return NewServices(t, t.composeProject)
}

func (t *Project) Tools() domain.Tools {
	if toolConfig, ok := t.Extensions[ToolExtensionName].(domain.Tools); ok {
		return toolConfig
	}
...
func findComposeFiles(projectFilePath string, config *domain.Config) ([]string, bool) {
	projectDirectory := filepath.Dir(projectFilePath)
